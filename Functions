...
def longest_subsequence_nlogn(coordinates):
    ...
    for i in range(1, n):
        # Binary search to find the position of current coordinate in ends list
        left, right = 0, length
        while left < right:
            mid = (left + right) // 2
...
def finding_heaviest_subsequence(coordinates):
    dp = [0] * len(coordinates)  # Initialize the dp table
    dp[0] = coordinates[0][2]  # Set the weight of the first point as the first value in the dp table

    parent = [-1] * len(coordinates)  # To keep track of the path

    # Fill up the dp table
    for i in range(1, len(coordinates)):
        for j in range(i):
            # Update dp[i] only if the x and y coordinates of j-th point are strictly less than the x and y
            # coordinates of i-th point
            if coordinates[j][1] < coordinates[i][1] and dp[i] < dp[j]:
                dp[i] = dp[j]
                parent[i] = j
        dp[i] += coordinates[i][2]
    ...
    # Retrieve the path
    path = []
    weight_sum = 0  # Initialize the sum of weights
    while max_index != -1:
        path.append(coordinates[max_index])
        weight_sum += coordinates[max_index][2]  # Add the weight of the current point to the sum of weights
        max_index = parent[max_index]
    path.reverse()

    return weight_sum, path
...
def update_weights_intelligence_algorithm(coordinates):
    ...
    while changeablePoints:
        ...
        new_weight, chain = finding_heaviest_subsequence(coordinates)
